"""Prompt templates for VideoReward — vendored verbatim from upstream
``prompt_template.py``. Builds the user-turn text from a generation
prompt + evaluation dimension(s), with several template flavours
(``none`` / ``simple`` / ``video_score`` / ``detailed`` /
``detailed_special``). The template_type chosen at training time is
also what the checkpoint's ``DataConfig.prompt_template_type`` captures.
"""
...
def build_prompt(prompt: str, dimension, template_type: str) -> str:
    """Render the user-turn text for a (prompt, dimension) pair.

    Args:
        prompt: Raw text prompt used to generate the video.
        dimension: Either a single dim name (``"VQ"`` / ``"MQ"`` / ``"TA"`` /
            ``"Overall"``) or a list of dim names. Multiple dims fold into
            an ``overall_performance(...)`` umbrella label.
        template_type: One of ``"none"`` / ``"simple"`` / ``"video_score"`` /
            ``"detailed"`` / ``"detailed_special"``. Must match the value
            captured in the checkpoint's ``DataConfig`` to keep the prompt
    ...
    raise ValueError(f"Invalid template type: {template_type!r}")
