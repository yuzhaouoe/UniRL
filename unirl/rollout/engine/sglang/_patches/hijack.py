...
Install once at the top of ``SGLangRolloutEngine.__init__`` -- BEFORE importing
...
class SglangDiffusionHijack:
    ...
    def hijack() -> None:
        ...
        from unirl.rollout.engine.sglang._patches.patch_dance import patch_dance
        from unirl.rollout.engine.sglang._patches.patch_denoising import (
            patch_denoising,
        )
        from unirl.rollout.engine.sglang._patches.patch_gpu_worker import (
            patch_gpu_worker,
        )
        from unirl.rollout.engine.sglang._patches.patch_grouped_dispatch import (
            patch_grouped_dispatch,
        )
        from unirl.rollout.engine.sglang._patches.patch_latent_prep import (
            patch_latent_prep,
        )
        from unirl.rollout.engine.sglang._patches.patch_lora_slice_2d import (
            patch_lora_slice_2d,
        )
        from unirl.rollout.engine.sglang._patches.patch_lora_tensors import (
            patch_lora_tensors,
        )
        from unirl.rollout.engine.sglang._patches.patch_pipeline import (
            patch_pipeline,
        )
        from unirl.rollout.engine.sglang._patches.patch_platform_device import (
            patch_platform_device,
        )
        from unirl.rollout.engine.sglang._patches.patch_rollout_trajectory import (
            patch_rollout_trajectory,
        )
        from unirl.rollout.engine.sglang._patches.patch_sampling_io import (
            patch_sampling_io,
        )
        from unirl.rollout.engine.sglang._patches.patch_scheduler import (
            patch_scheduler,
        )
        from unirl.rollout.engine.sglang._patches.patch_sd3_lora_pipeline import (
            patch_sd3_lora_pipeline,
        )
        from unirl.rollout.engine.sglang._patches.patch_set_timesteps import (
            patch_set_timesteps,
        )
        from unirl.rollout.engine.sglang._patches.patch_srt import patch_srt
        from unirl.rollout.engine.sglang._patches.patch_vae_decode_safe import (
